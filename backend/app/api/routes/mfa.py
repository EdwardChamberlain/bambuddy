...
async def setup_totp(
    ...
    provisioning_uri = totp.provisioning_uri(name=current_user.username, issuer_name="Grove Control")
    ...
    return TOTPSetupResponse(secret=secret, qr_code_b64=qr_b64, issuer="Grove Control")
...
async def enable_email_otp(
    ...
    try:
        send_email(
            ...
            subject="Verify your Grove Control email address for 2FA",
            body_text=(
                f"Your Grove Control email 2FA setup code is: {code}\n\n"
                "Enter this code to confirm email-based two-factor authentication.\n"
                "The code expires in 10 minutes."
            ),
            body_html=(
                "<p>To enable <strong>email-based two-factor authentication</strong> on your Grove Control account, "
...
async def send_email_otp(
    ...
    try:
        send_email(
            smtp_settings=smtp_settings,
            to_email=user.email,
            subject="Your Grove Control verification code",
            body_text=f"Your Grove Control login code is: {code}\n\nThis code expires in {UserOTPCode.OTP_TTL_MINUTES} minutes and can only be used once.",
            body_html=(
                f"<p>Your <strong>Grove Control</strong> login verification code is:</p>"
                f"<h2 style='letter-spacing:4px'>{code}</h2>"
...
async def refresh_oidc_provider_icon(
    ...
    Used when:
    - The IdP changed its icon and the admin wants Grove Control to pick up the
      new bytes.
