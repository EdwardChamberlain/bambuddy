...
async def add_to_queue(
    ...
    # Batch progress is keyed by the queue item's plate_id. Cross-model
    # requests carry the plate on each candidate rather than on the top-level
    # request, so mirror the first candidate until the scheduler resolves the
    # winning variant (which then writes the selected plate onto the item).
    queue_plate_id = data.plate_id
    if queue_plate_id is None and variant_specs:
        queue_plate_id = variant_specs[0][0].plate_id
    ...
    if batch_id is None and quantity > 1:
        ...
        batch_id = batch.id
        if variant_specs:
            # Cross-model submissions do not pre-create an order through the
            # frontend, so give quantity>1 the same planned-demand semantics as
            # the multi-plate flow. Failed/cancelled alternatives remain owed.
            db.add(
                PrintBatchPlate(
                    batch_id=batch.id,
