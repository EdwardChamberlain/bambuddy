...
class PrinterManager:
    ...
    async def _broadcast_status_change(self, printer_id: int) -> None:
        ...
        ``awaiting_plate_clear`` flag, but any future Grove Control-side flag added
        ...
        deduplicates on a status_key that intentionally excludes Grove Control
        ...
        try:
            ...
            logger.warning(
                "Failed to broadcast printer_status after Grove Control-side state change for printer %d: %s",
                printer_id,
                e,
            )
    ...
    def set_print_running_observed_callback(self, callback: Callable[[int, dict], None]):
        ...
        printer that started its print before Grove Control came up — the #1304
...
def resolve_plate_id(state) -> int | None:
    ...
    printer card falls back to plate 1 — wrong thumbnail. When Grove Control
    ...
    from a previous Grove Control-dispatched print bleeding into a Studio-direct
