...
class TestPushStatusCache:
    ...
    async def test_net_info_ip_rewritten_for_unknown_secondary_interface(self):
        ...
        active interfaces (WiFi + Ethernet) — only ONE matches the IP Grove Control
        ...
        h2d_le = _ip_to_uint32_le(H2D_IP)
        # A second IP Grove Control never saw (e.g. printer's ethernet interface
        # while Grove Control talks over wifi).
    ...
    async def test_tray_exist_bits_clears_empty_slots_in_slicer_cache(self):
        ...
        cleanup that bambu_mqtt.py applies to Grove Control's internal state, the
