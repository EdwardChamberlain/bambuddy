...
@pytest.mark.asyncio
async def test_entity_listing_uses_pinned_lan_transport():
    """Entity discovery must use the same DNS-pinned transport as HA control."""
    service = HomeAssistantService()
    client = _mock_get([])
    transport = object()
    ...
    with (
        ...
        await service.list_entities("http://ha.local", "tok")

    async_client.assert_called_once_with(timeout=service.timeout, transport=transport, trust_env=False)


@pytest.mark.asyncio
async def test_sensor_entity_listing_uses_pinned_lan_transport():
    """Sensor discovery must not bypass the shared LAN SSRF protection."""
    service = HomeAssistantService()
    client = _mock_get([])
    transport = object()
