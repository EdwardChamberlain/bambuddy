...
def test_sheet_starting_position_offsets_the_first_label():
    with patch("backend.app.services.label_renderer._draw_label") as draw_label:
        render_labels("avery_5160", [_sample()], starting_position=8)

    args = draw_label.call_args.args
    assert args[1] == pytest.approx(4.76 * mm + (66.675 * mm + 3.175 * mm))
...
def test_invalid_sheet_starting_position_raises(template, starting_position):
