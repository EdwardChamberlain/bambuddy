...
export function PrinterQueueWidget({ printerId, printerModel, loadedFilamentTypes, loadedFilaments, variant = 'card' }: PrinterQueueWidgetProps) {
  ...
  const linkClassName = variant === 'panelExtension'
    ? 'block min-w-0 w-full max-w-full overflow-hidden mt-2 border-t border-bambu-dark-tertiary pt-2 pl-1 hover:opacity-90 transition-opacity'
    : 'block min-w-0 w-full max-w-full overflow-hidden mb-3 p-3 bg-bambu-dark rounded-lg hover:bg-bambu-dark-tertiary transition-colors';
      ...
      <div className="flex min-w-0 max-w-full items-center justify-between gap-3 overflow-hidden">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p
              className="block min-w-0 max-w-full truncate text-sm text-white"
