export default {
  ...
  nav: {
    ...
    installAppSuccess: 'Grove Controlをインストールしました',
  },
  ...
  printers: {
    ...
    maintenance: {
      ...
      editFieldHelp: '有効にすると、このプリンターはMQTT、キューディスパッチ、通知から一時停止されます — 修理、並列のGrove Controlインストール、または一時的な停止に役立ちます。',
    },
    // Chamber light
  ...
  archives: {
    ...
    no3mfBanner: {
      ...
      body: 'スライサーがプリンターのSDカードに.gcode.3mfファイルを残さなかったため、Grove Controlはサムネイルもスライサーメタデータも取得できませんでした。これは通常、スライサー (Bambu Studio / OrcaSlicer のデバイスタブ) で「送信ファイルを外部ストレージに保存」がオフになっていることが原因です。',
  ...
  settings: {
    ...
    ldap: {
      ...
      feature3: 'ログイン時にLDAPグループがGrove Controlグループにマッピングされます',
      ...
      autoProvisionHint: '初回LDAPログイン時にGrove Controlアカウントを自動作成',
      ...
      groupMappingHint: 'LDAPグループDNをGrove Controlグループにマッピング。利用可能なグループ: ',
    ...
    manageProjectsDescription: 'プロジェクトの作成、更新、削除、およびアーカイブの追加を行います。印刷をプロジェクトで整理する自動化に適しています。',
    projectsBadge: 'プロジェクト',
    manageArchives: 'アーカイブを管理',
    manageArchivesDescription: '古い印刷を含む印刷アーカイブを編集・削除します。統計への寄与を削除する操作は含まれません。',
    archivesBadge: 'アーカイブ',
    ...
    concurrentUploadsTitle: 'プリンターへの同時アップロード',
    concurrentUploadsDescription: '同時にファイルを受信できる独立したプリンターの数を選択します。1 は従来の直列動作を維持し、値を大きくすると複数プリンターへの送信が速くなりますが、ホストの接続数と帯域幅を多く使用します。',
    concurrentUploadsLabel: '進行中のプリンターセッション',
    concurrentUploadsHelp: '既定値は 1 です。各項目には、プールの空きスロットまたはプリンターの予約を待っているかが表示されます。',
    ...
    authDisabledDescription: '認証を有効にして、ユーザーアカウントの作成、権限の管理、Grove Controlインスタンスのセキュリティを確保しましょう。',
    authDisabledMessage: '認証を有効にして、ユーザーアカウントの作成、権限の管理、Grove Controlインスタンスのセキュリティを確保しましょう。',
    ...
    disableAuthenticationMessage: '認証を無効にしますか？Grove Controlインスタンスにログインなしでアクセスできるようになります。ユーザーはデータベースに残りますが、認証は無効になります。',
    disableAuthentication: '認証を無効化',
    // Additional settings
    configureGroveControl: 'Grove Controlを設定',
    ...
    captureFinishPhotoDescription: '印刷完了時にプリンターカメラから写真を撮影します。Grove Control は印刷中に短いタイムラプスを記録し、ベッドが下がる前の瞬間から写真を取得できるようにします。この印刷でタイムラプスを有効にしていた場合はタイムラプスファイルが保存され、それ以外の場合は写真の取得後に自動的に削除されます。',
    ...
    groveControlSoftware: 'Grove Controlソフトウェア',
    autoCheckDescription: '起動時に自動的に新しいバージョンを確認',
    checkNow: '今すぐ確認',
    updateAvailableVersion: 'アップデート利用可能: v{{version}}',
    releaseNotes: 'リリースノート',
    updateViaDocker: 'Docker Composeでアップデート:',
    updateViaHomeAssistant: 'アップデートはHome Assistant Supervisorによって管理されます。Home Assistantの設定→アドオン→Grove Controlを開いて新しいバージョンをインストールしてください。',
    ...
    // Network tab
    externalUrl: '外部URL',
    externalUrlDescription: 'Grove Controlがアクセス可能な外部URL。通知画像や外部連携に使用されます。',
    groveControlUrl: 'Grove ControlのURL',
    ...
    mqttDescription: 'Node-RED、Home Assistant、その他の自動化システムとの統合のため、外部MQTTブローカーにGrove Controlイベントをパブリッシュ。',
    ...
    encryption: {
      ...
      notConfiguredDesc: 'TOTPシークレットとOIDCのclient_secretsは平文で保存されています。MFA_ENCRYPTION_KEYを設定するか、書き込み可能なデータディレクトリでGrove Controlを再起動して自動生成してください。',
      ...
      migrationErrorWarning: '{{count}} 件のレガシー行を起動時に再暗号化できませんでした。サーバーログを確認し、Grove Control を再起動して再試行してください。',
    },
  ...
  login: {
    ...
    resetStep1: 'Grove Control管理者に連絡',
    ...
    twoFA: {
      ...
      instructionsTotp: '認証アプリを開いて、Grove Control用の6桁のコードを入力してください。',
  ...
  // Setup page
  setup: {
    title: 'Grove Control セットアップ',
    subtitle: 'Grove Controlインスタンスの認証を設定',
    enableAuth: '認証を有効化',
  ...
  users: {
    title: 'ユーザー管理',
    subtitle: 'ユーザーとGrove Controlインスタンスへのアクセスを管理',
    backToSettings: '設定に戻る',
    createUser: 'ユーザーを作成',
    noPermission: 'このページにアクセスする権限がありません。',
    admin: '管理者',
  ...
  profiles: {
    ...
    orcaCloud: {
      ...
      connect: {
        title: 'Orca Cloudに接続',
        description: 'Orca Cloudアカウントにサインインして、スライサープロファイルをGrove Controlに同期します。',
      },
      providers: {
        google: 'Googleでサインイン',
        apple: 'Appleでサインイン',
  ...
  inventory: {
    ...
    spoolmanMixedContentBody: 'Grove Control はリバースプロキシ経由で HTTPS 配信されていますが、Spoolman の URL は HTTP のままです。ブラウザはセキュリティ上の理由で混在コンテンツをブロックするため、埋め込みの Spoolman UI を表示できません。Spoolman も HTTPS でアクセスできる必要があります。',
    spoolmanMixedContentFixReverseProxy: 'Spoolman を Grove Control と同じリバースプロキシ（Traefik / Nginx / Caddy）の後ろに HTTPS で配置し、設定で Spoolman URL を新しい HTTPS アドレスに更新してください。',
  ...
  printModal: {
    ...
    filamentOverride: 'フィラメント要件',
    filamentOverrideHint: '通常はスライス済みプロファイルを使用します。必要な場合のみ色または互換素材のサブタイプを変更してください。',
    selectFilamentSlot: '装填済みスロットを選択',
    manuallySelected: '手動で選択',
    automaticallyMatched: '自動で一致',
  ...
  backup: {
    ...
    repoIsPublicWarning: 'リポジトリが公開（PUBLIC）です。Grove Controlのバックアップには MQTT 認証情報、Home Assistant トークン、Prometheus トークン、Bambu Cloud のメールアドレス、K-プロファイル経由のプリンタアクセスコードが含まれます。プロバイダー側でリポジトリをプライベートに変更するまで保存はブロックされます。',
    repoVisibilityUnknown: 'リポジトリの公開設定を確認できませんでした。Grove Control はプライベートと確認できないリポジトリへのバックアップを拒否します。保存はブロックされます。',
    ...
    appSettings: 'アプリ設定',
    appSettingsDescription: 'Grove Control設定（データベース全体）',
    ...
    localBackupDescription: 'データベース、アーカイブ、アップロード、すべてのファイルを含むGrove Controlデータの完全なバックアップを作成します。',
    ...
    backupRestoredRestart: 'バックアップが復元されました。Grove Controlを再起動してください。',
  ...
  kProfiles: {
    ...
    modal: {
      ...
      notesHelp: 'メモはGrove Controlに保存され、プリンターには保存されません',
      syncing: 'プリンターと同期中...',
  ...
  virtualPrinter: {
    ...
    enable: {
      title: '仮想プリンターを有効化',
      visibleInSlicer: 'スライサーの検出リストに「Grove Control」として表示',
    ...
    remoteInterface: {
      ...
      hint: 'SSDPで広告され、TLS証明書に使用されるIPアドレスを上書きします。Grove Controlに複数のネットワークインターフェースがある場合に便利です。',
    },
    mode: {
      title: 'モード',
      archive: 'アーカイブ',
    ...
    caCert: {
      title: 'スライサー証明書',
      description: '仮想プリンターは Grove Control CA が署名した TLS 証明書を使用します。この CA 証明書をスライサーの信頼ストアに一度インポートすると接続が受け入れられます — コマンドラインからコピーする必要はありません。',
      copy: 'コピー',
      copied: 'コピーしました',
      download: 'ダウンロード',
      fingerprint: 'SHA-256',
  ...
  spoolbuddy: {
    ...
    settings: {
      ...
      backendUrl: 'Grove ControlバックエンドURL',
      apiToken: 'APIトークン',
  ...
  diagnostic: {
    ...
    check: {
      ...
      external_storage: {
        ...
        skip_unsupported_model: '未確認 — このプリンターモデルではこのオプションを有効にできません。インストール手順4を手動で確認してください。',
      },
      port_rtsps: {
      ...
      network_mode: {
        ...
        warn: 'Grove ControlはDockerブリッジネットワークで実行されています。プリンター検出と仮想プリンターにはホストネットワークモードが必要です — "network_mode: host" でコンテナを再作成してください。',
        skip: 'Dockerで実行されていません — 該当しません。',
      },
      subnet: {
        title: 'ネットワークサブネット',
        pass: 'プリンターとGrove Controlは同じサブネットにあります。',
        warn: 'プリンター ({{printer_ip}}) とGrove Control ({{host_ip}}) は異なるサブネットにあります。サブネット間のルーティングが設定されていない限り、互いに到達できない可能性があります。',
  ...
  systemHealth: {
    ...
    signature: {
      'ftp-auth-rejected': {
        ...
        fix: 'プリンター画面（LAN設定）からアクセスコードをコピーし直し、Grove Control のプリンター設定で更新してください。',
      },
      'ftp-connection-timeout': {
        name: 'ファイル転送の接続がタイムアウトしました',
        cause: 'Grove Control がプリンターのファイル転送ポート（FTPS 990）に到達できませんでした。ポートがブロックされているか、プリンターの電源が切れているか、別のサブネットにあります。',
        fix: 'Grove Control とプリンターの間でポート 990 がブロックされていないこと、両者が同じネットワークにあることを確認してください。',
      },
      'ftp-ssl-error': {
      ...
      'database-locked': {
        ...
        fix: 'Grove Control を外部の PostgreSQL データベースに切り替えてください。ドキュメントの PostgreSQL ガイドを参照してください。',
      },
    },
  },
  ...
  vpDiagnostic: {
    ...
    check: {
      ...
      running: {
        title: 'サービスが稼働中',
        fail: '仮想プリンターは有効ですが、サービスが稼働していません。Grove Control のログを確認してください — 通常はバインド IP の競合または権限エラーが原因で停止します。',
      },
      bind_interface: {
      ...
      certificate: {
        title: 'TLS 証明書',
        pass: '証明書の準備ができています。Grove Control CA 証明書（上記）がスライサーの信頼ストアにインポートされていることを確認してください。',
        fail: 'この仮想プリンターの TLS 証明書がありません。Grove Control のデータディレクトリが書き込み可能か確認してください。',
      },
    },
  },
  failureDetection: {
    ...
    externalUrlHint: 'ML APIはURLでカメラスナップショットを取得します。MLAPIコンテナがGrove Controlに到達できるよう、一般設定で外部URLを設定してください。',
  ...
  makerworld: {
    title: 'MakerWorld',
    description: 'MakerWorld モデルの URL を貼り付けると、Bambu Handy アプリを開かなくても Grove Control から直接インポート・印刷できます。',
    ...
    disclaimer: 'MakerWorld 連携はコミュニティで文書化された API エンドポイントを使用しています。Grove Control は MakerWorld または Bambu Lab との提携・承認関係はありません。',
  ...
  libraryPurge: {
    ...
    effect4: '外部（リンク済み）フォルダー内のファイルはスキップされます — Grove Controlは所有していないバイトを削除しません。',    previewLoading: '対象ファイル数を確認中…',
